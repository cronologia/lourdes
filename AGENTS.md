...
touching any `data/*.json` — plus **data-edit**, bootstrap-project, mine-video,
dossier-research; in a checkout they live at `core/skills/sourcing-rules/` and
`core/skills/data-edit/`); the architecture rationale in `cronologia/fsp` →
`docs/adrs/`.

## What this project is

A compiled static website documenting the chronology of **Nossa Senhora de
Lourdes (Our Lady of Lourdes)** — the apparitions reported by Bernadette
Soubirous at the Massabielle grotto in 1858 and the Catholic Church's
documented response.
...
The five rules of the `sourcing-rules` skill (cronologia/core) apply verbatim.
**Every data edit goes through the `data-edit` gate**: query, edit,
`node scripts/validate-data.js`, `node --test`, `node build.js`, commit data
plus regenerated `docs/` in the same change. Never hand-edit `docs/`.

Subject-specific rules:

1. **Apparitions are REPORTED events with Church judgments.** The dataset
   records who reported what and when, and what the Church ruled and when,
   citing the ruling document. It never asserts the supernatural claim as
   fact — event prose says "Bernadette reports…", never "the Lady said…"
   in the site's own voice.
2. **Church-approval status is a critical field.** The approval of Lourdes is
   a specific, dated, quotable act: Bishop Bertrand-Sévère Laurence's decree
   of 18 January 1862, following the commission he opened on 28 July 1858.
   Keep the decree quoted and cited; do not paraphrase "approved" without it.
3. **Quote the 25 March 1858 phrase only as reported.** The Gascon Occitan
   "Que soy era Immaculada Concepciou" is quoted with the spelling of the
   source cited; spellings vary — see `KEYWORDS.md` before searching for it.
4. **Cures: the datable event is the recognition act.** Record the bishop's
   declaration (who, when, which cure), with the running count attributed to
   the Sanctuary of Lourdes — never "X miracles happened". The Medical
   Bureau's "unexplained" finding and the bishop's "miraculous" declaration
   are different acts by different bodies; keep them distinct.
5. **The Sanctuary's own pages disagree on the cure count** (70 on the
   lourdes-france.org page vs 72 on lourdes-france.com, as of the 2026-08-05
   bootstrap). Record disagreements instead of resolving them; both pages are
   kept in `references[]` deliberately.
6. **Preferred primaries:** the Sanctuary of Lourdes' own site
   (lourdes-france.com / lourdes-france.org) and vatican.va. Encyclopedias
   fill biographical gaps and are labeled; testimony and devotional sites are
